...
CREATE POLICY "Users can view own bookings" ON bookings FOR SELECT USING (auth.uid() = user_id OR auth.uid() IN (SELECT supplier_id FROM listings WHERE id = booking_id));
